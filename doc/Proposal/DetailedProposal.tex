...
\begin{document}
    ...
    \section{Development Plan}
        ...
        \begin{enumerate}
            ...
            \item step through directory, check settings from configuration file, either store/don't store packages appropriately.  for specific options see ``Commands to be Implementation'' below%options are: \begin{itemize}
                %\item store everything (\verb+all+)
                %\item prompt user (\verb+ask+)
                %\item ignore everything (\verb+never+)
                %\item check a list of ``store only these'' (\verb+neverexcept+)
                %\item ``store these and ask about others'' (\verb+askexcept+)
                %\item ``store all but these''
                %\item ``don't store these, ask about others''
            %\end{itemize}
